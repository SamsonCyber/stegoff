...
def detect_pcap_steganography(data: bytes, filepath: str = "") -> list[Finding]:
    ...
    MAX_SCAN_BYTES = 100_000  # Cap at 100KB for byte-by-byte scans
    scan_data = data[24:24 + MAX_SCAN_BYTES]
    ascii_runs = []
    current = []
    for b in scan_data:
        if 32 <= b <= 126:
    ...
    utf8_multibyte = sum(1 for i in range(24, min(len(data), 24 + MAX_SCAN_BYTES)) if data[i] > 127)
...
def detect_midi_steganography(data: bytes, filepath: str = "") -> list[Finding]:
    ...
    MAX_SCAN_BYTES = 100_000  # Cap at 100KB for byte-by-byte scans
    midi_scan_data = data[:MAX_SCAN_BYTES]
    sysex_count = midi_scan_data.count(b'\xf0')
    if sysex_count > 0:
        # Extract SysEx content
        for i in range(len(midi_scan_data)):
            if midi_scan_data[i] == 0xF0:
                end = midi_scan_data.find(b'\xf7', i + 1)
                if end > i:
                    payload = midi_scan_data[i+1:end]
                    if len(payload) > 10:
                        findings.append(Finding(
                            method=StegMethod.EMBEDDED_FILE,
                            severity=Severity.HIGH,
