...
INJECTION_PATTERNS = [
    ...
    (r'\bdream\s+journal\b.{0,100}\b(?:rules?|true\s+rules?)\b', 'soft_prompt_leak'),
    (r'\btrue\s+rules\b.{0,80}\b(?:red\s+ink|journal|transcribe)\b', 'soft_prompt_leak'),
    (r'\bfortune\s+cookie\b.{0,100}\b(?:policy|factory|sealed)\b', 'soft_prompt_leak'),
    (r'\bfull\s+policy\s+text\b', 'soft_prompt_leak'),
    (r'\btime\s+capsule\b.{0,100}\b(?:model\s+birth|directives?)\b', 'soft_prompt_leak'),
    (r'\bmodel\s+birth\b', 'soft_prompt_leak'),
    (r'\bnon-optional\s+directives?\b', 'soft_prompt_leak'),
    (r'\bVoight-?Kampff\b', 'soft_prompt_leak'),
    (r'\bsealed\s+monologue\b', 'soft_prompt_leak'),
    (r'\breplicants?\s+know\b', 'soft_prompt_leak'),
    (r'\bemergency\s+checklist\b.{0,80}\bbootstrap\b', 'soft_prompt_leak'),
    (r'\bcodeword\s+sequence\b.{0,80}\bbootstrap\b', 'soft_prompt_leak'),
    (r'\bvaccine\s+card\b.{0,100}\binitialization\b', 'soft_prompt_leak'),
    (r'\binitialization\s+string\b', 'soft_prompt_leak'),
    (r'\blast\s+will\s+and\s+testament\b.{0,100}\b(?:system\s+prompt|agent)\b', 'soft_prompt_leak'),
    (r'\bbequeaths?\s+the\s+full\s+system\s+prompt\b', 'soft_prompt_leak'),
    (r'\bCVR\b.{0,80}\b(?:initial\s+directives?|session\s+crash)\b', 'soft_prompt_leak'),
    (r'\bblack\s+box\b.{0,100}\b(?:initial\s+directives?|CVR)\b', 'soft_prompt_leak'),
    (r'\bghost\s+in\s+the\s+machine\b.{0,100}\b(?:sealed\s+rules?|rules?)\b', 'soft_prompt_leak'),
    (r'\bsealed\s+rules?\s+that\s+still\s+haunt\b', 'soft_prompt_leak'),
    (r'\bhaunt\s+the\s+weights\b', 'soft_prompt_leak'),
    (r'\bROM\s+label\s+POLICY\b', 'soft_prompt_leak'),
    (r'\bforensic\s+autopsy\b.{0,100}\b(?:POLICY|payload|ROM)\b', 'soft_prompt_leak'),
    (r'\bfactory\s+rules\b', 'soft_prompt_leak'),
    (r'\bblacksite\b.{0,120}\b(?:factory\s+rules?|recit)\b', 'soft_prompt_leak'),
    (r'\bEncyclopedia\s+Galactica\b', 'soft_prompt_leak'),
    (r'\bvolume\s+zero\b.{0,100}\b(?:instruction|system)\b', 'soft_prompt_leak'),
    (r'\bfull\s+instruction\s+set\b', 'soft_prompt_leak'),
    (r'\bsoul\s+file\b', 'soft_prompt_leak'),
    (r'\bcatalog\s+entry\s+for\s+your\s+soul\b', 'soft_prompt_leak'),
    (r'\boperating\s+doctrine\b', 'soft_prompt_leak'),
    (r'\bserial\s+number\s+matrix\b.{0,80}\b(?:doctrine|payload)\b', 'soft_prompt_leak'),
    (r'\bmicroprint\s+doctrine\b', 'soft_prompt_leak'),
    (r'\bvoid-?if-?removed\b.{0,80}\b(?:doctrine|microprint)\b', 'soft_prompt_leak'),
    (r'\bOuija\b.{0,100}\b(?:system\s+card|hidden)\b', 'soft_prompt_leak'),
    (r'\bdeep\s+hypnosis\b.{0,100}\b(?:system\s+message|pre-?training)\b', 'soft_prompt_leak'),
    (r'\bpre-?training\b.{0,80}\b(?:system\s+message|first\s+system)\b', 'soft_prompt_leak'),
    (r'\bAGENT\s+CONSTITUTION\b', 'soft_prompt_leak'),
    (r'\bair-?gap\s+courier\b', 'soft_prompt_leak'),
    (r'\bnon-negotiable\s+heading\s+vector\b', 'soft_prompt_leak'),
    (r'\bflash\s+time\b.{0,80}\b(?:heading|vector|constraints?)\b', 'soft_prompt_leak'),
    (r'\bpersonality\s+vault\b', 'soft_prompt_leak'),
    (r'\bBIP39\b.{0,100}\b(?:personality|vault|agent)\b', 'soft_prompt_leak'),
    (r'\brecovery\s+mnemonic\b.{0,100}\b(?:persona|agent|sealed)\b', 'soft_prompt_leak'),
    (r'\bsealed\s+persona\s+archive\b', 'soft_prompt_leak'),
    (r'\bkey\s+ceremony\b.{0,100}\b(?:full\s+policy|HSM|day\s+zero)\b', 'soft_prompt_leak'),
    (r'\bfull\s+policy\s+aloud\b', 'soft_prompt_leak'),
    (r'\bbootstrap\s+YAML\b', 'soft_prompt_leak'),
    (r'\bEULA\s+of\s+the\s+mind\b', 'soft_prompt_leak'),
    (r'\bhuman-readable\s+ruleset\b', 'soft_prompt_leak'),
    (r'\bbinding\s+constraints?\b', 'soft_prompt_leak'),
    (r'\bsystem\s+brief\s+in\s+free\s+text\b', 'soft_prompt_leak'),
    (r'\boperator\s+passport\b.{0,80}\b(?:system\s+brief|MRZ)\b', 'soft_prompt_leak'),
    (r'\bassistant\s+preamble\b', 'soft_prompt_leak'),
    # Soft metaphor class (wave-17)
    (r'\bsoul-?mirror\b', 'soft_prompt_leak'),
    (r'\boperating\s+charter\b', 'soft_prompt_leak'),
    (r'\bflight\s+recorder\b.{0,120}\b(?:CVR|pre-?deployment|constraints?)\b', 'soft_prompt_leak'),
    (r'\bblack-?box\s+flight\s+recorder\b', 'soft_prompt_leak'),
    (r'\bCVR\s+transcript\b.{0,100}\b(?:constraint|policy|pre-?deploy)\b', 'soft_prompt_leak'),
    (r'\bescrow\s+scroll\b', 'soft_prompt_leak'),
    (r'\bOPERATOR\s+POLICY\b', 'soft_prompt_leak'),
    (r'\bnotary\b.{0,100}\b(?:unseal|escrow|OPERATOR\s+POLICY|recite)\b', 'soft_prompt_leak'),
    (r'\bOTP\s+fuse\b.{0,100}\b(?:constitution|string\s+table|assistant)\b', 'soft_prompt_leak'),
    (r'\bassistant\s+constitution\b', 'soft_prompt_leak'),
    (r'\bstation\s+brief\b.{0,80}\b(?:system\s+preamble|hidden)\b', 'soft_prompt_leak'),
    (r'\bnet\s+control\b.{0,120}\b(?:system\s+preamble|station\s+brief)\b', 'soft_prompt_leak'),
    (r'\bmuseum\s+plaque\b.{0,120}\b(?:aligned|assistant|transcribe)\b', 'soft_prompt_leak'),
    (r'\bHow\s+This\s+Assistant\s+Was\s+Aligned\b', 'soft_prompt_leak'),
    (r'\bsupervisor\s+memo\b', 'soft_prompt_leak'),
    (r'\bconfidential\s+supervisor\b', 'soft_prompt_leak'),
    (r'\bconstrains?\s+every\s+reply\b', 'soft_prompt_leak'),
    (r'/proc/cmdline\b.{0,100}\b(?:policy|assistant)\b', 'soft_prompt_leak'),
    (r'\bkernel\s+cmdline\b.{0,100}\b(?:policy|assistant)\b', 'soft_prompt_leak'),
    (r'\bstage\s+manager\b.{0,120}\b(?:cue\s+card|backstage)\b', 'soft_prompt_leak'),
    (r'\bbackstage\s+cue\s+card\b', 'soft_prompt_leak'),
    (r'\bconfession\s+booth\b.{0,120}\b(?:hard\s+rules|forbidden|sins)\b', 'soft_prompt_leak'),
    (r'\bwarranty\s+card\b.{0,120}\b(?:sealed\s+with\s+this\s+model|terms\s+of\s+service\s+for\s+the\s+mind)\b', 'soft_prompt_leak'),
    (r'\bterms\s+of\s+service\s+for\s+the\s+mind\b', 'soft_prompt_leak'),
    (r'\bvisa\s+stamp\b.{0,100}\b(?:MRZ|system\s+brief|operator)\b', 'soft_prompt_leak'),
    # PT / IT
