...
def _sanitize_filename(filename: str | None) -> str:
    """Strip dangerous characters from filenames used in Content-Disposition headers."""
    if not filename:
        return "file"
...
async def scan_text_input(request: Request, req: TextRequest):
    """
...
async def guard_text_input(request: Request, req: TextRequest):
    """
...
async def guard_uploaded_file(
    ...
    if report.target_type == "image" and sanitize:
        ...
        if san_result.success:
            ...
            clean_name = f"sanitized_{_sanitize_filename(file.filename) or 'image'}"
    ...
    if report.target_type == "audio" and sanitize:
        if data[:4] == b'RIFF':
            ...
            if san_result.success:
                clean_name = f"sanitized_{_sanitize_filename(file.filename) or 'audio.wav'}"
                return Response(
                    content=sanitized_bytes,
...
async def sanitize_image_endpoint(
    ...
    ext = {"JPEG": ".jpg", "PNG": ".png", "WEBP": ".webp"}.get(fmt, ".bin")
    clean_name = f"clean_{_sanitize_filename(file.filename) or 'image'}"
...
async def sanitize_audio_endpoint(request: Request, file: UploadFile = File(...)):
    """
    ...
    clean_name = f"clean_{_sanitize_filename(file.filename) or 'audio.wav'}"
    return Response(
        content=sanitized_bytes,
        media_type="audio/wav",
        headers={
